...
// Looked up by device_id alone (unique across all users) — needed for the
// biometric returning-user flow, where the client may not have a valid
// session token at all yet, only its device_id and a fresh signature.
export async function getDeviceByDeviceIdOnly(deviceId) {
  const { rows } = await query(
...
export async function getOrCreateDevice({ userEmail, userName, deviceId, name, platform, secret }) {
  const existing = await getDevice(userEmail, deviceId);
  if (existing) return existing;
  const { rows } = await query(
    `INSERT INTO devices (user_email, user_name, device_id, name, platform, secret)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [userEmail, userName, deviceId, name, platform, secret]
  );
  return rows[0];
}

export async function createDevice({ userEmail, userName, deviceId, name, platform, secret }) {
  const { rows } = await query(
    `INSERT INTO devices (user_email, user_name, device_id, name, platform, secret)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [userEmail, userName, deviceId, name, platform, secret]
...
export async function setBiometricPublicKey(deviceId, publicKey) {
  await query(
    `UPDATE devices SET biometric_public_key = $2, biometric_enrolled_at = now() WHERE device_id = $1`,
    [deviceId, publicKey]
  );
}
