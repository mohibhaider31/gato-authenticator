...
import { getDeviceIdFromReq, setDeviceIdCookie } from "./deviceCookie";
import { getBearerToken, verifyMobileToken, createMobileToken } from "./mobileToken";
import { getDevice, newDeviceId } from "./deviceStore";
...
export async function getAuthContext(req, res) {
  ...
  if (isMobile) {
    ...
    // The device cookie is normally set once at login, but a browser could
    // lose just that cookie (privacy settings, manual clearing, an old
    // session predating this cookie's introduction) while keeping the
    // login session valid. Rather than leaving that request permanently
    // stuck with no device_id and no path to get one, self-heal here.
    if (rawUser && !deviceId) {
