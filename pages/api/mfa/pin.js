  ...
  try {
    ...
    if (!device) {
      ...
      try {
        ...
        // Duplicate device_id (a near-simultaneous double-submit racing to
        // create the same device row) — the row exists now regardless of
        // which request created it, so just fetch it instead of failing.
        if (err.code === "23505") {
    ...
    if (!device) return res.status(500).json({ error: "device_creation_failed" });
    ...
    console.error("[api/mfa/pin] unexpected error:", err);
